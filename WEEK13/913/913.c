...
void phim_inv()
{
   if (!input(inv) && dcmotor_onoff)
   {
      ...
      if (!input(inv))
      {
         ...
         step = 0;
         dcmotor_tn = dcmotor_tn ^ 1;
         if (dcmotor_onoff) {dc_motor_stop(); delay_ms(1000); }
         xuat_32led_don_2word((dcmotor_tn?0xff:0)|,(!dcmotor_tn?0xff:0));
      }
   }
}
