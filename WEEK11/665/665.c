...
void main()
{
   ...
   while(TRUE)
   {
      ...
      else 
      {
         bdn = bdn -10;
         dem_tg_exit++;
         if (dem_tg_exit == 20) mode = 0; giay++; draw(giay); temp = temp +read_adc(); try++; delay_quet_8led(1);
         if (try == 100) {lm35 = temp /2.046; temp = lm35 / 100;}
