...
int8 dem = 0;
int16 temp = 0, lm35a = 0, temp1 = 0, lm35b = 0;
int try = 0;
int1 ttqn = 0;
float M,N;
...
void giai_ma(){
   ...
   printf(lcd_data,"%ld",lm35a);
   printf(lcd_data," %ld",lm35b);
}
...
void LCD_DISPLAY_COUNTER(){
   ...
   LCD_SO_LON(D,1,0);
   LCD_SO_LON(C,1,3);
   xuat_4led_7doan_4so(ma7doan[count/10],ma7doan[count%10],ma7doan[lm35a/10],ma7doan[lm35a%10]);
}
