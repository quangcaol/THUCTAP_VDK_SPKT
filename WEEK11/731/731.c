...
float k_cach, clk_cach, kcs, temperature;
...
void BaoChay(){
   ...
   if (ds18a > 33) xuat_32led_don_2word(0,0xFFFF);
   if (ds18a < 31) xuat_32led_don_2word(0xFFFF,0);
}
...
void giai_ma(){
   ...
   printf(lcd_data,"TEMPERATURE: %.4f",temperature);
}
...
void readDS() {
      ...
      if (ds18al != ds18a_tam) {
         ds18a_tam = ds18al;
         temperature = ds18a/16.0;
         unsigned int16 real = temperature*100;
         xuat_4led_7doan_4so(ma7doan[real/1000],ma7doan[(real/100)%10]&0x7F,ma7doan[(real/10)%10],ma7doan[real%10]);
      }
}
